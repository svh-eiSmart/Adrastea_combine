...
#define CALC_CRC(x) crc = tab[((crc >> 24) ^ (x)) & 255] ^ (crc << 8)
...
unsigned long crc32_calc(unsigned long crc, const uint8_t *buf, uint32_t len) {
  const uint32_t *tab = crc32_table;
  const uint32_t *b = (const unsigned long *)buf;
  size_t rem_len;
  ...
  if (len) {
    ...
    do {
      CALC_CRC(*++p); /* use pre increment for speed */
    } while (--len);
  }
...
uint32_t crc32(uint32_t crc, const char *p, uint32_t len) {
  return crc32_calc(crc ^ 0xffffffffL, (const uint8_t *)p, len) ^ 0xffffffffL;
}

void calc_sha256_sw(uint8_t *start_addr, int data_len, uint8_t *out_addr) {
  sha256_memory(start_addr, data_len, out_addr);
}
...
otp_sb_enable_status_e DRV_SB_Enable_MCU(uint32_t offset) {
...
#endif
  {
    ...
    calcChecksum = crc32(0, (const char *)buffer, SB_PUK2_SIZE);
