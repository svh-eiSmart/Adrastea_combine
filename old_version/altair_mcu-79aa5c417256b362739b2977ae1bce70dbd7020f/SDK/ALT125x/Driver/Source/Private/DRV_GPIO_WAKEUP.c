...
#if (configUSE_ALT_SLEEP == 1)
int32_t DRV_GPIO_WAKEUP_HandleSleepNotify(sleep_notify_state sleep_state, void* ptr_ctx) {
...
void DRV_GPIO_WAKEUP_Initialize(void) {
  ...
  if (!is_initialized) {
    ...
    ret_val = sleep_notify_register_callback(&DRV_GPIO_WAKEUP_HandleSleepNotify,
                                             &temp_for_sleep_notify, NULL);
