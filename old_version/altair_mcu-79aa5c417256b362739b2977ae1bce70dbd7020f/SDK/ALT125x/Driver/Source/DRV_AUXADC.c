...
#define AUX_ADC_VBAT_MAILBOX_ADDR 0x0D02FD80
typedef struct {
  AUX_ADC_Type *base;
...
typedef struct {  // the address for the struct is 0xBD02FD80
  uint16_t resolution;
  uint16_t selref;
  uint32_t num_of_samples;
  uint32_t vbat_meas_result;
  uint32_t temp_meas_result;
} aux_adc_info_and_results;

static aux_adc_info_and_results *vbat_mailbox =
    (aux_adc_info_and_results *)AUX_ADC_VBAT_MAILBOX_ADDR;
...
int DRV_AUXADC_Get_Bat_Sns(void) {
  ...
  res = vbat_mailbox->resolution;
  v_ref = vbat_mailbox->selref;

  /*perform computations as described in mini-SRS of auxadc*/
  D = vbat_mailbox->vbat_meas_result / vbat_mailbox->num_of_samples;
  result = (D * v_ref * 3) / res;
  return result;
}

int32_t DRV_AUXADC_Get_Temperature(void) { return DRV_PMP_AGENT_GetTemperature(); }
