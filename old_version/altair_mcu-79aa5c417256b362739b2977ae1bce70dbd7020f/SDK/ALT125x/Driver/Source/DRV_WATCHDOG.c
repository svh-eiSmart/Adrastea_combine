...
#if (CONFIG_WATCHDOG_SUPPORT_PM == 1)
static int32_t DRV_WATCHDOG_SleepHandler(sleep_notify_state state, void *arg) {
  if (state == SUSPENDING) {
...
#if (CONFIG_WATCHDOG_SUPPORT_PM == 1)
  if (sleep_notify_register_callback(DRV_WATCHDOG_SleepHandler, &wdt_peripheral.sleep_notify_inx,
                                     NULL) != 0)
...
#if (CONFIG_WATCHDOG_SUPPORT_PM == 1)
  if (sleep_notify_remove_callback_item(wdt_peripheral.sleep_notify_inx) != 0)
    return DRV_WATCHDOG_ERROR_PM;
#endif
