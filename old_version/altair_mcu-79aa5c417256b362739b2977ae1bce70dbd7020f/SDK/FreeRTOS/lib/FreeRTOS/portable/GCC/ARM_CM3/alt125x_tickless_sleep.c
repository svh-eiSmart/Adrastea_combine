...
void vPortSuppressTicksAndSleep(TickType_t xExpectedIdleTime) {
  ...
  TickType_t ExpectedStopTime = xExpectedIdleTime;
#endif
  PWR_MNGR_Configuation conf = {0}; 

  pwr_mngr_conf_get_settings(&conf);
  PWR_MNGR_PwrMode pwr_mode = conf.mode;
  ...
  __asm volatile("isb");
#if (configUSE_ALT_SLEEP == 1)
  if (pwr_mode != PWR_MNGR_MODE_SLEEP) DRV_PM_MaskInterrupts(pwr_mode);
#endif
  ...
  if (eTaskConfirmSleepModeStatus() == eAbortSleep) {
    ...
    above. */
#if (configUSE_ALT_SLEEP == 1)
    if (pwr_mode != PWR_MNGR_MODE_SLEEP) DRV_PM_RestoreInterrupts();
#endif
...
#if (configUSE_ALT_SLEEP == 1)
    if (pwr_mode != PWR_MNGR_MODE_SLEEP) {
      pwr_mngr_pre_sleep_process(xModifiableIdleTime);
    }
...
#ifdef PWR_MNGR_MAX_STOP_DRUATION
          if ((PWR_MNGR_MAX_STOP_DRUATION != 0) && (ExpectedStopTime > PWR_MNGR_MAX_STOP_DRUATION)) {
            ExpectedStopTime = PWR_MNGR_MAX_STOP_DRUATION;
          }
          pwr_mngr_conf_set_mode(PWR_MNGR_MODE_STOP, ExpectedStopTime);
          xExpectedIdleTime = ExpectedStopTime;
#else 
          xExpectedIdleTime = conf.duration;
#endif
        }
    ...
    if (pwr_mode != PWR_MNGR_MODE_SLEEP) {
      pwr_mngr_post_sleep_process(xModifiableIdleTime);
    }
#endif
    ...
    if (pwr_mode == PWR_MNGR_MODE_STOP) {
      ...
      if (DRV_PM_GetStatistics(&statistics) == 0) {
        if (xExpectedIdleTime > statistics.last_dur_left) {
          ...
          xExpectedIdleTime = (DRV_32KTIMER_GetValue() * 1000 + 16384) / 32768;
        }
      }
    }
#endif
