...
static void log_term_cb(uint8_t profile, uint8_t err, char *token) {
  APITEST_DBG_PRINT("\n[CB_TERM_URC]\n");
  APITEST_DBG_PRINT("Profile: %d\n", profile);
  APITEST_DBG_PRINT("Err: %d\n", err);
  if (token) APITEST_DBG_PRINT("token: %s\n", token);
  APITEST_DBG_PRINT("--------\n");
}

static void log_rst_cb(uint8_t profile, char *token) {
  APITEST_DBG_PRINT("\n[CB_RST_URC]\n");
  APITEST_DBG_PRINT("Profile: %d\n", profile);
  if (token) APITEST_DBG_PRINT("token: %s\n", token);
  APITEST_DBG_PRINT("--------\n");
...
static void log_cmd_cb(uint8_t profile, void *userPriv) {
  ...
  if (len) APITEST_DBG_PRINT("Token: %s\n", urc_data->token);
...
static void apitest_task(FAR void *arg) {
  ...
  while (1) {
    ...
    if (command && command->argc >= 1) {
      ...
      if (!contextinit) memset(&context, 0, sizeof(context));
      if (!cmdparaminit) memset(&CmdParams, 0, sizeof(CmdParams));
      ...
      exit_config:
        if (context.coapCfgIp.dest_addr) free(context.coapCfgIp.dest_addr);
        if (context.coapCfgDtls.CipherList) free(context.coapCfgDtls.CipherList);
        if (context.coapCfgDtls.CipherListFilteringType)
          free(context.coapCfgDtls.CipherListFilteringType);
        if (context.coapCfgIp.sessionId) free(context.coapCfgIp.sessionId);
        contextinit = 0;
      ...
      /**********  CMD DATA/ OPTIONS  *****************/
        ...
        /* Register callbacks */
        coap_EventRegister((void *)log_cmd_cb, COAP_CB_CMDS);
        coap_EventRegister((void *)log_rst_cb, COAP_CB_CMDRST);
        coap_EventRegister((void *)log_term_cb, COAP_CB_CMDTERM);
      ...
      command_exit:
        if (CmdParams.uri) free(CmdParams.uri);
        if (CmdParams.token) {
          if (CmdParams.token->token) free(CmdParams.token->token);
          free(CmdParams.token);
        }
        if (CmdParams.content) free(CmdParams.content);
        if (CmdParams.blk_size) free(CmdParams.blk_size);
        if (CmdParams.block_num) free(CmdParams.block_num);
        if (CmdParams.block_m) free(CmdParams.block_m);
        if (CmdParams.data) free(CmdParams.data);
        int i;
        for (i = 0; i < CmdParams.optionsArgc; i++) {
          if (!CmdParams.optionsArgV) break;
          if (CmdParams.optionsArgV[i]) free(CmdParams.optionsArgV[i]);
          if (CmdParams.optionsArgV[i]->option_value) free(CmdParams.optionsArgV[i]->option_value);
        }
        if (CmdParams.optionsArgV) free(CmdParams.optionsArgV);
        cmdparaminit = 0;
      }
    }
