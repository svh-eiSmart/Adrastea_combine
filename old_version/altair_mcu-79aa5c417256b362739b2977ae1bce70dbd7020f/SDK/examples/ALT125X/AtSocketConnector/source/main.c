...
static int32_t sleep_callback(sleep_notify_state state, void* arg) {
  if (state == SUSPENDING) {
...
extern void prepare_saving_context();
extern int resume_last_context();
...
static void init_task(void* arg) {
  ...
  if (DRV_PM_GetDevBootType() == DRV_PM_DEV_COLD_BOOT) {
    prepare_saving_context();
  } else {
    resume_last_context();
  }
...
int main(void) {
  ...
  DRV_GPIO_Initialize();
  DRV_GPIO_PowerControl(POWER_FULL);
