    ...
    if (command && command->argc >= 1) {
      ...
      if (cmp_res == 0) {
        if (3 == command->argc) {
          ...
          if (!strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_REQUESTED_ARG,
      ...
      if (cmp_res == 0) {
        if (3 <= command->argc && 0 == strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_NULL_ARG,
                                               strlen(APITEST_NULL_ARG))) {
          print_retval(lte_set_report_netstate(NULL, NULL),
                       APITEST_GETFUNCNAME(lte_set_report_netstat));
        } else {
      ...
      if (cmp_res == 0) {
        if (3 <= command->argc && 0 == strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_NULL_ARG,
          ...
          print_retval(lte_set_report_simstate(NULL, NULL),
                       APITEST_GETFUNCNAME(lte_set_report_simstat));
        } else {
      ...
      if (cmp_res == 0) {
        if (3 == command->argc) {
          if (!strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_DISABLE_ARG,
                       strlen(APITEST_NULL_ARG))) {
            print_retval(lte_set_report_regstate(NULL, NULL),
                         APITEST_GETFUNCNAME(lte_set_report_regstate));
          } else if (!strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_ENABLE_ARG,
                              strlen(APITEST_NULL_ARG))) {
      ...
      if (cmp_res == 0) {
        if (3 == command->argc) {
          if (!strncmp(command->argv[APITEST_CMD_PARAM_1], APITEST_DISABLE_ARG,
            ...
            APITEST_DBG_PRINT("ERROR: unexpected parameter\n");
            goto errout_with_param;
          }
        } else {
          APITEST_DBG_PRINT("ERROR: unexpected parameter number %d\n", command->argc);
          goto errout_with_param;
        }
      }
      ...
      if (cmp_res == 0) {
        if (2 == command->argc) {
          print_retval(lte_set_report_scanresult(scanresult_report_cb, (void *)0xdeadbeef),
                       APITEST_GETFUNCNAME(lte_set_report_fw_upgrade_evt));
        } else {
          APITEST_DBG_PRINT("unexpected parameter number %d\n", command->argc);
          goto errout_with_param;
        }
