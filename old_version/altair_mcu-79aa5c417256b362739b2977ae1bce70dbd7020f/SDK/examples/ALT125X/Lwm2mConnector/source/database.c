...
#define MAX_OBJ19_0_0_INSTNUM 1
 ...
 ****************************************************************************/
typedef int (*databaseReadIndCB_t)(int instanceNum, int resourceNum, int *resourceInst,
                                   LWM2MRsrc_type_e *valType, LWM2MInstnc_type_e *instType,
                                   resourceVal_t *resourceVal, void *userPriv);
...
static uint8_t sampledata[MAX_OBJ19_0_0_SIZE] = {0x48, 0x65, 0x6C, 0x6C, 0x6F,
...
static int dbObj6Read(int instId, int rsrcId, int *rsrcInst, LWM2MRsrc_type_e *valType,
                      LWM2MInstnc_type_e *instType, resourceVal_t *resourceVal, void *userPriv) {
  APP_DBG_PRINT("[DB] Read /6/%d/%d/%d\r\n", instId, rsrcId, *rsrcInst);

  *valType = RSRC_VALUE_TYPE_FLOAT;
  *instType = RSRC_INSTNC_TYPE_SINGLE;
  *rsrcInst = 0;
  switch (rsrcId) {
    case 0:
      resourceVal->value = mylocation.Latitude;
      APP_DBG_PRINT("[DB] Get Latitude %f\r\n", resourceVal->value);
      break;

    case 1:
      resourceVal->value = mylocation.Longitude;
      APP_DBG_PRINT("[DB] Get Longitude %f\r\n", resourceVal->value);
      break;

    case 2:
      resourceVal->value = mylocation.Altitude;
      APP_DBG_PRINT("[DB] Get Altitude %f\r\n", resourceVal->value);
      break;

    case 3:
      resourceVal->value = mylocation.Radius;
      APP_DBG_PRINT("[DB] Get Radius %f\r\n", resourceVal->value);
      break;

    case 4:
      *valType = RSRC_VALUE_TYPE_OPAQUE;
      resourceVal->opaqueVal = mylocation.Velocity.rsrc[0].opaque;
      APP_DBG_PRINT("[DB] Get Velocity, length: %hu, ptr: 0x%p\r\n",
                    (uint16_t)resourceVal->opaqueVal.opaqueValLen,
                    (void *)resourceVal->opaqueVal.opaqueValPtr);
      break;

    case 5:
      *valType = RSRC_VALUE_TYPE_TIME;
      resourceVal->value = mylocation.Timestamp;

      time_t dataCreateTime;
      char timeStr[32];

      dataCreateTime = (int)resourceVal->value;
    ...
    case 6:
      resourceVal->value = mylocation.Speed;
      APP_DBG_PRINT("[DB] Get Speed %d\r\n", (int)resourceVal->value);
      break;
...
static int dbObj19Read(int instId, int rsrcId, int *rsrcInst, LWM2MRsrc_type_e *valType,
                       LWM2MInstnc_type_e *instType, resourceVal_t *resourceVal, void *userPriv) {
  /* TODO:// Add multiple instance handling */
  int i;
  app_data_container_inst_t *tmpobj = NULL;

  APP_DBG_PRINT("[DB] Read /19/%d/%d/%d\r\n", instId, rsrcId, *rsrcInst);
  ...
  switch (rsrcId) {
    case 0: {
      *valType = RSRC_VALUE_TYPE_OPAQUE;
      *instType = RSRC_INSTNC_TYPE_MULTIPLE;
      *rsrcInst = 1;
      resourceVal->opaqueVal.opaqueValLen =
          (unsigned short)mydatacontainer.obj[0].data.rsrc[0].opaque.opaqueValLen;
      resourceVal->opaqueVal.opaqueValPtr =
          (unsigned char *)mydatacontainer.obj[0].data.rsrc[0].opaque.opaqueValPtr;
    ...
    case 1: {
      ...
      *instType = RSRC_INSTNC_TYPE_SINGLE;
      *rsrcInst = 0;
      resourceVal->value = (double)mydatacontainer.obj[0].dataPriority;
    ...
    case 2: {
      ...
      *instType = RSRC_INSTNC_TYPE_SINGLE;
      *rsrcInst = 0;
      resourceVal->value = (double)mydatacontainer.obj[0].dataCreateTime;
    ...
    case 3: {
      ...
      *instType = RSRC_INSTNC_TYPE_SINGLE;
      *rsrcInst = 0;
      snprintf(resourceVal->strVal, MAX_LWM2M_STRING_TYPE_LEN, "%s",
               mydatacontainer.obj[0].dataDesc);
    ...
    case 4: {
      ...
      *rsrcInst = 0;
      snprintf(resourceVal->strVal, MAX_LWM2M_STRING_TYPE_LEN, "%s",
               mydatacontainer.obj[0].dataFormat);
    ...
    case 5: {
      ...
      *instType = RSRC_INSTNC_TYPE_SINGLE;
      *rsrcInst = 0;
      resourceVal->value = (double)mydatacontainer.obj[0].appId;
      APP_DBG_PRINT("[DB] Get appId %hu\r\n", (uint16_t)resourceVal->value);
      break;
    }
...
static int dbObj19Write(int instId, int rsrcId, int rsrcInst, LWM2MInstnc_type_e instncType,
  ...
  switch (rsrcId) {
    case 0: {
      ...
      APP_ASSERT(tmpobj->data.rsrc[rsrcInst].opaque.opaqueValPtr != NULL,
                 "reportMngrBindReadIndApi failed\r\n");
    ...
    case 2: {
      ...
      tmpobj->dataCreateTime = atoi(resourceVal);
      dataCreateTime = (time_t)tmpobj->dataCreateTime;
      if (CTIME_R(&dataCreateTime, timeStr)) {
...
static int dbObj4Read(int instId, int rsrcId, int *rsrcInst, LWM2MRsrc_type_e *valType,
                      LWM2MInstnc_type_e *instType, resourceVal_t *resourceVal, void *userPriv) {
  APP_DBG_PRINT("[DB] Read /4/%d/%d/%d\r\n", instId, rsrcId, *rsrcInst);
  *valType = RSRC_VALUE_TYPE_INTEGER;
  *instType = RSRC_INSTNC_TYPE_MULTIPLE;
  *rsrcInst = 0;
  switch (rsrcId) {
    case 3:
      resourceVal->value = myconninfo.link_quality;
      APP_DBG_PRINT("[DB] Get link_quality %d\r\n", (int)resourceVal->value);
      break;

    case 8:
      resourceVal->value = myconninfo.glob_cell_id;
      APP_DBG_PRINT("[DB] Get glob_cell_id %d\r\n", (int)resourceVal->value);
      break;

    case 9:
      resourceVal->value = myconninfo.mnc;
...
int database_init(void) {
  ...
  /* Assign /6/0/4 instId */
  mylocation.Velocity.rsrc[0].instId = 0;
  /* Assign /6/0/4 opaqque length */
  mylocation.Velocity.rsrc[0].opaque.opaqueValLen = sizeof(sampledata);
  /* Allocate for /6/0/4 opaque data */
  mylocation.Velocity.rsrc[0].opaque.opaqueValPtr = malloc(sizeof(sampledata));
  APP_ASSERT(mylocation.Velocity.rsrc[0].opaque.opaqueValPtr != NULL, "malloc failed\r\n");

  /* Copy sample data into /6/0/4 opaque data */
  memcpy((void *)mylocation.Velocity.rsrc[0].opaque.opaqueValPtr, (void *)sampledata,
         sizeof(sampledata));
  /* Assign /6/0/5 */
  mylocation.Timestamp = (int)time(NULL);
  ...
  for (i = 0; i < MAX_OBJ19_INSTNUM; i++) {
    ...
    for (j = 0; j < MAX_OBJ19_0_0_INSTNUM; j++) {
      ...
      mydatacontainer.obj[i].data.rsrc[j].opaque.opaqueValLen = sizeof(sampledata);
      /* Allocate for /19/i/0/j opaque data */
      mydatacontainer.obj[i].data.rsrc[j].opaque.opaqueValPtr = malloc(sizeof(sampledata));
      APP_ASSERT(mydatacontainer.obj[i].data.rsrc[j].opaque.opaqueValPtr != NULL,
                 "malloc failed\r\n");

      /* Copy sample data into /19/i/0/j opaque data */
      memcpy((void *)mydatacontainer.obj[i].data.rsrc[j].opaque.opaqueValPtr, (void *)sampledata,
             sizeof(sampledata));
    }
    ...
    /* Assign /19/i/2 */
    mydatacontainer.obj[i].dataCreateTime = (int)time(NULL);
    /* Assign /19/i/3 */
...
int database_read(int objId, int instId, int rsrcId, int *rsrcInst, LWM2MRsrc_type_e *valType,
                  LWM2MInstnc_type_e *instType, resourceVal_t *resourceVal, void *userPriv) {
  ...
  return cbPtr(instId, rsrcId, rsrcInst, valType, instType, resourceVal, userPriv);
}
