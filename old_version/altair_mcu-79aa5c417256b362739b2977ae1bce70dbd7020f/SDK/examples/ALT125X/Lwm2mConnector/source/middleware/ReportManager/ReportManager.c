...
static void dump_resource(lwm2m_uri_and_data_t *uriData) {
  ...
  switch (uriData->valType) {
    case RSRC_VALUE_TYPE_FLOAT:
      APP_DBG_PRINT("/%lu/%lu/%lu/%lu, type: %lu, val: %f\r\n", uriData->objectId,
                    uriData->instanceNum, uriData->resourceNum, uriData->resourceInstanceNum,
                    (uint32_t)uriData->valType, uriData->resourceVal.value);
      break;

    case RSRC_VALUE_TYPE_INTEGER:
    case RSRC_VALUE_TYPE_TIME:
    case RSRC_VALUE_TYPE_BOOLEAN:
      APP_DBG_PRINT("/%lu/%lu/%lu/%lu, type: %lu, val: %d\r\n", uriData->objectId,
                    uriData->instanceNum, uriData->resourceNum, uriData->resourceInstanceNum,
                    (uint32_t)uriData->valType, (int32_t)uriData->resourceVal.value);
      break;

    case RSRC_VALUE_TYPE_STRING:
      APP_DBG_PRINT("/%lu/%lu/%lu/%lu, type: %lu, val: %s\r\n", uriData->objectId,
                    uriData->instanceNum, uriData->resourceNum, uriData->resourceInstanceNum,
                    (uint32_t)uriData->valType, uriData->resourceVal.strVal);
      break;

    case RSRC_VALUE_TYPE_OPAQUE:
      APP_DBG_PRINT("/%lu/%lu/%lu/%lu type: %lu, len: %hu\r\n", uriData->objectId,
                    uriData->instanceNum, uriData->resourceNum, uriData->resourceInstanceNum,
                    (uint32_t)uriData->valType, uriData->resourceVal.opaqueVal.opaqueValLen);
      break;
...
static int reportMngr_read_report_cb(client_inst_t client, int objectId, int instanceNum,
                                     int resourceNum, int *resourceInst, LWM2MRsrc_type_e *valType,
  ...
  LWM2MInstnc_type_e instType;
  // TODO: Add instance type usage

  return cbPtr(objectId, instanceNum, resourceNum, resourceInst, valType, &instType, resourceVal,
               userPriv);
}
...
#if 0 /* Skip /19/X/0/Y temporarily */
    int i;
    for (i = 0; i < reqContainer->reportToBeSend->u.lwm2m_app_data.numInst; i++) {
      numOfResrcToRreport +=
          (reqContainer->reportToBeSend->u.lwm2m_app_data.obj[i].data.numInst - 1);
    }
#endif
