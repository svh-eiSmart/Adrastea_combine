...
#define DEMO_SLEEP_DURATION (60000)
...
static int32_t pwrAwsDemo_sleep_notify(sleep_notify_state pm_state, void *ptr_ctx);
/****************************************************************************
 * Private Types
 ****************************************************************************/
typedef enum {
...
uint32_t got_sleep_notify = 0;
bool useFakeGps = false;
GnssInfo_t fakeGnssInfo = {0.0, 2450.513400, 'N', 12101.142800, 'E', 0};
bool forceGpsRenew = false;

time_t gLocalTime __attribute__((section("GPMdata")));
char gLocation[128] __attribute__((section("GPMdata")));
/****************************************************************************
 * Private Functions
 ****************************************************************************/
...
static void dtchnet_dummycb(lteresult_e result, void *pCtx) {
  ...
  /* Turn off gps */
  /* altcom_gps_activate(ALTCOM_GPSACT_GPSOFF, ALTCOM_GPSACT_NOTOL);
...
static appMainLoopState_e INIT_INIT_Handler(void *pCtx, void *pParam) {
  ...
  /*Config GPIO and interrupt handler */
...
static appMainLoopState_e LTEDTCH_GNSSSTART_Handler(void *pCtx, void *pParam) {
  ...
  /* Start GPS */
  ret = altcom_gps_activate(ALTCOM_GPSACT_GPSON, ALTCOM_GPSACT_NOTOL);
...
static appMainLoopState_e GNSSWAIT_GNSSREADY_Handler(void *pCtx, void *pParam) {
  ...
  /* Turn off gps */
  ret = altcom_gps_activate(ALTCOM_GPSACT_GPSOFF, ALTCOM_GPSACT_NOTOL);
  APP_ASSERT(0 == ret, "altcom_gps_activate fail, ret = %d\n", ret);
...
static appMainLoopState_e ANY_EXPIRY_Handler(void *pCtx, void *pParam) {
  ...
  APP_DBG_PRINT("We are going to sleep now!!\n");
  smngr_dev_busy_clr(pwrawsdemo_smngr_sync_id);

  /* Waiting for sleep notify and enter to sleep */
  while (got_sleep_notify == 0) {
    vTaskDelay(DEMO_SLEEP_DURATION); /* only effect to stop mode */
  }

  /* Suppose we have done everything after enter into sleep state and never come back in phase 0*/
  return INIT_INIT_Handler(pCtx, (void *)BTN_GPIO_NUM);
  // For current official RK, we just go to IDLE //return STATE_SLEEP;
}
...
static void appMainLoopTask(void *arg) {
  ...
  while (ctx->taskRunning) {
    ...
    APP_DBG_PRINT("currState: %d, evt: %d\n", (int)ctx->currState, (int)evt.event);
    if (ctx->stateTransMatrix[ctx->currState][evt.event]) {
      ...
      if (osTimerIsRunning(ctx->sleepTimer)) {
        ...
        APP_ASSERT(osOK == ret, "timer stop fail, ret=%d\n", ret);
      }

      ret = osTimerStart(ctx->sleepTimer, ctx->expiryTimeoutTable[ctx->currState]);
      APP_ASSERT(osOK == ret, "timer change period fail, ret=%d\n", ret);
    } else {
      APP_DBG_PRINT("evt: %d ignored in currState: %d\n", (int)evt.event, ctx->currState);
    }

    /* Release event element */
    APP_DBG_PRINT("nextState: %d\n", (int)ctx->currState);
  }
...
static int32_t pwrAwsDemo_sleep_notify(sleep_notify_state pm_state, void *ptr_ctx) {
  if (pm_state == SUSPENDING) {
    got_sleep_notify = 1;  // received sleep notify - going to sleep
  }
