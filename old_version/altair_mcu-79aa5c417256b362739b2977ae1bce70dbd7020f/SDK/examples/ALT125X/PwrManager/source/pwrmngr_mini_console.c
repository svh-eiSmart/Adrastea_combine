...
int32_t do_pwrMode(char *s) {
  ...
  if (!strcmp(command, "stop")) {
    printf("Configured to Stop mode.\r\n");
#ifdef PWR_MNGR_MAX_STOP_DRUATION
    printf(
        "Note: now Stop duration is calculated by system. User input duration will not be "
        "used!\r\n");
#endif
    if (pwr_mngr_conf_set_mode(PWR_MNGR_MODE_STOP, duration) != PWR_MNGR_OK) printf("Failed.\r\n");
...
int32_t do_oslessStop(char *s) {
  ...
  DRV_PM_MaskInterrupts(PWR_MNGR_MODE_STOP);
...
int32_t do_oslessStandby(char *s) {
  ...
  // (1) disable interrupts
  __asm volatile("cpsid i" ::: "memory");
  DRV_PM_MaskInterrupts(PWR_MNGR_MODE_STANDBY);
  // (2) optional - add system specific pre sleep procedure here
...
int32_t do_oslessShutdown(char *s) {
  ...
  // (1) disable interrupts
  __asm volatile("cpsid i" ::: "memory");
  DRV_PM_MaskInterrupts(PWR_MNGR_MODE_SHUTDOWN);
  // (2) optional - add system specific pre sleep procedure here
...
int32_t do_setContRetention(char *s) {
  /*  note: ALT1250 has 2 sections; ALT1255 has only one section.
      1:bank-1
      2:bank-2
      3:(bank1|bank2)
  */

  uint32_t sec_id_mask = PWR_CON_RETENTION_SEC_MASK_ALL;
  int32_t argc = 0;
  char *tok, *strgp;
  ...
  sec_id_mask = (uint32_t)strtoul(tok, NULL, 10);
  argc++;

  if (pwr_mngr_enable_retention_sec(sec_id_mask) != PWR_MNGR_OK) {
    printf("Failed to configure Content Retention section ID mask %lu\r\n", sec_id_mask);
    return (-1);
  } else {
    printf("Configured Retention section ID mask to %lu\r\n", sec_id_mask);
  }
...
static void wdt_timeout_handler(void *user_param) {
  ...
  printf(
      "Watchdog timeout!!! Prevent go to sleep. System will be reset on next timeout (ETA "
      "%lds).\r\n",
      wd_time);
