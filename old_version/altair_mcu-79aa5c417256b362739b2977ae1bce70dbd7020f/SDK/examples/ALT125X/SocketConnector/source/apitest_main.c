...
extern void start_tcpserver(uint16_t port);
extern void start_udpclient(char *ip, uint16_t port);
extern void start_udpserver(uint16_t port);
    ...
    if (command && command->argc >= 1) {
      ...
      if (cmp_res == 0) {
        if (4 == command->argc) {
          int sockfd;
          int sock_ret;
          struct altcom_sockaddr_in server;
          char nctaskname[32];
          char c;
          osStatus_t ncret;

          /* open socket */
          sockfd = altcom_socket(ALTCOM_AF_INET, ALTCOM_SOCK_STREAM, ALTCOM_IPPROTO_TCP);
          print_retval(sockfd < 0, APITEST_GETFUNCNAME(altcom_socket));
          if (sockfd >= 0) {
            /* configure server */
            memset(&server, 0, sizeof(struct altcom_sockaddr_in));
            sock_ret = altcom_inet_pton(ALTCOM_AF_INET, command->argv[APITEST_CMD_PARAM_1],
                                        (void *)&server.sin_addr.s_addr);
            if (1 != sock_ret) {
              goto errout_with_param;
            }

            server.sin_family = ALTCOM_AF_INET;
            server.sin_port = altcom_htons(atoi(command->argv[APITEST_CMD_PARAM_2]));

            /* connect */
            sock_ret = altcom_connect(sockfd, (struct altcom_sockaddr *)&server,
      ...
      if (!strncmp(command->argv[APITEST_CMD_ARG], "tcp", strlen("tcp"))) {
        if (!strncmp(command->argv[APITEST_CMD_PARAM_1], "client", strlen("client"))) {
          ...
          if (4 == command->argc) {
            start_tcpserver(atoi(command->argv[APITEST_CMD_PARAM_2]));
          }
        } else {
          APITEST_DBG_PRINT("iperf tcp <client ip port|server port>\n");
          goto errout_with_param;
        }
      }
      ...
      if (!strncmp(command->argv[APITEST_CMD_ARG], "udp", strlen("udp"))) {
        if (!strncmp(command->argv[APITEST_CMD_PARAM_1], "client", strlen("client"))) {
          ...
          if (4 == command->argc) {
            start_udpserver(atoi(command->argv[APITEST_CMD_PARAM_2]));
          }
        } else {
          APITEST_DBG_PRINT("iperf udp <client ip port|server port>\n");
          goto errout_with_param;
        }
      }
