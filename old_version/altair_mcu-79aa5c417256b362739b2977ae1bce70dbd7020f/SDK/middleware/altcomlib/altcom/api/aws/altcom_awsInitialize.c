...
alt_osal_queue_handle gAwsEvtQue[MQTT_EVT_UNSCONF + 1];
...
AWSError_e awsCheckApiStatus(alt_osal_queue_handle *que, int32_t timeout_ms) {
  MQTTError_e ret;
  ...
  if (alt_osal_recv_mqueue(que, (int8_t *)&ret, sizeof(MQTTError_e), timeout_ms) !=
      sizeof(MQTTError_e)) {
    ...
    for (i = MQTT_EVT_CONCONF; i <= MQTT_EVT_UNSCONF; i++) {
      ...
      if (ret != 0) {
        ...
        for (j = MQTT_EVT_CONCONF; j < i; j++) {
          alt_osal_delete_mqueue(&gAwsEvtQue[j]);
        }
