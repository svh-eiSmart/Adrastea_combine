...
/**
 ...
 @params [in] http_tls_config      HTTP URI configuration parameters.
...
Http_err_code_e altcom_http_tls_config(Http_profile_id_e profile_id,
  ...
  if (http_tls_config->CipherList != NULL) sendLen += strlen(http_tls_config->CipherList) + 1;
  ...
  sendLen += HTTP_CONFIG_TLS_REQ_DATALEN - HTTP_CONFIG_CIPHER_MAXLEN;
  ...
  if (http_tls_config->CipherList) {
    ...
    if (HTTP_CONFIG_CIPHER_MAXLEN > strLen) {
      strLen = snprintf(str, HTTP_CONFIG_CIPHER_MAXLEN, "%s", http_tls_config->CipherList);
