...
char *find_nmask_pos(char *src, lteapniptype_e ip_type) {
...
int conv_dot2colon_v6(const char *src, char *dst) {
  if (!src || !dst) {
    return -1;
  }
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (pdn_info->pdn[i].is_valid) {
        ...
        if (pdn_info->pdn[i].ip_type == LTE_APN_IPTYPE_IP) {
          ...
          if (conv_dot2colon_v6(res->pdninfo[i].ip_nmask_str, pdn_info->pdn[i].ip_str) < 0) {
            pdn_info->pdn[i].ip_str[0] = '\0';
          }

          if (conv_dot2colon_v6(res->pdninfo[i].gw_str, pdn_info->pdn[i].gw_str) < 0) {
            pdn_info->pdn[i].gw_str[0] = '\0';
          }

          if (conv_dot2colon_v6(res->pdninfo[i].pri_dns_str, pdn_info->pdn[i].pri_dns_str) < 0) {
            pdn_info->pdn[i].pri_dns_str[0] = '\0';
          }

          if (conv_dot2colon_v6(res->pdninfo[i].sec_dns_str, pdn_info->pdn[i].sec_dns_str) < 0) {
