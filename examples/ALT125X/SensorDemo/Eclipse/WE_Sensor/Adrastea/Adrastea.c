 ...
 */
static WE_Pin_t Adrastea_pins[Adrastea_Pin_Count] = {
		0 };
...
bool Adrastea_Init(uint32_t baudrate, WE_FlowControl_t flowControl, WE_Parity_t parity, Adrastea_EventCallback_t eventCallback, WE_Pin_t *pins)
{
	...
	if (NULL == pins)
	{
		...
		Adrastea_GetDefaultPinConfig(Adrastea_pins);
	}
	else
	{
		/* Use the supplied pin config */
		for (uint8_t i = 0; i < Adrastea_Pin_Count; i++)
		{
			Adrastea_pins[i] = pins[i];
		}
	}

	if (false == WE_InitPins(Adrastea_pins, Adrastea_Pin_Count))
	{
		/* Error */
		return false;
	}
	...
	WE_SetPin(Adrastea_pins[Adrastea_Pin_WakeUp], WE_Pin_Level_Low);
	WE_SetPin(Adrastea_pins[Adrastea_Pin_Reset], WE_Pin_Level_High);

	WE_UART_Init(baudrate, flowControl, parity, true); /* Adrastea default: 115200 Baud 8 */
	WE_Delay(10);

	/* reset module */
	if (Adrastea_PinReset())
	{
	...
	if (revisionIdentity.major != Adrastea_firmwareVersionMajor || revisionIdentity.minor != Adrastea_firmwareVersionMinor)
	{
		printf("Driver version and module firmware version mismatch, driver might not behave as expected\r\n");
	}
...
bool Adrastea_Deinit(void)
{
	...
	for (int i = 0; i < Adrastea_Pin_Count; i++)
	{
		WE_DeinitPin(Adrastea_pins[i]);
	}
...
{
	if (!WE_SetPin(Adrastea_pins[Adrastea_Pin_Reset], WE_Pin_Level_Low))
	{
		return false;
	}
	WE_Delay(100);
	return WE_SetPin(Adrastea_pins[Adrastea_Pin_Reset], WE_Pin_Level_High);
}
...
bool Adrastea_PinWakeUp(void)
{
	...
	if (!WE_SetPin(Adrastea_pins[Adrastea_Pin_WakeUp], WE_Pin_Level_High))
	{
		return false;
	}
	...
	if (!WE_SetPin(Adrastea_pins[Adrastea_Pin_WakeUp], WE_Pin_Level_Low))
	{
		return false;
	}
...
{
	if (pin >= Adrastea_Pin_Count)
	{
		return false;
	}
	return WE_SetPin(Adrastea_pins[pin], level);
}
...
WE_Pin_Level_t Adrastea_GetPinLevel(Adrastea_Pin_t pin)
{
	...
	return WE_GetPinLevel(Adrastea_pins[pin]);
}
...
bool Adrastea_SendRequest(char *data)
{
	 ...
	 * and the next command sent to the module is not shorter than Adrastea_minCommandIntervalUsec */
...
{
	pins[Adrastea_Pin_Reset].port = GPIOA;
	pins[Adrastea_Pin_Reset].pin = GPIO_PIN_10;
	pins[Adrastea_Pin_Reset].type = WE_Pin_Type_Output;
	pins[Adrastea_Pin_WakeUp].port = GPIOA;
	pins[Adrastea_Pin_WakeUp].pin = GPIO_PIN_9;
	pins[Adrastea_Pin_WakeUp].type = WE_Pin_Type_Output;
}
